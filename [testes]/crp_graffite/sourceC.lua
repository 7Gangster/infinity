...
addCommandHandler("grafiti",
	function ()
		showTheEditor()
	end
)

addCommandHandler("grafiti2",
	function (command, size)
		if not tonumber(size) or tonumber(size) <= 0 or tonumber(size) > 5 then
			outputChatBox("#b0e56b[san Pixar]:#ffffff /" .. command .. " [Size (1 - 5)]", 255, 255, 255, true)
		else
			graffitiSize = tonumber(size)
			outputChatBox("#b0e56b[san Pixar]:#ffffff The graffiti size sucessfully changed to #e56b6b" .. graffitiSize .. ".", 255, 255, 255, true)
		end
	end
)

addCommandHandler("grafiti3",
	function ()
...
addCommandHandler("gclean",
	function ()
		...
		if graffitiCleanMode then
			outputChatBox("#b0e56b[san Pixar]: #ffffffGraffiti cleaning mode is now enabled.", 255, 255, 255, true)
		else
			outputChatBox("#e56b6b[san Pixar]: #ffffffGraffiti cleaning mode is now disabled.", 255, 255, 255, true)
		end
	end
)
				...
				if deltaTime >= 3000 then
					if utf8.len(path) > 0 then
						...
						if fileExtension and fileExtension == ".png" or fileExtension == ".jpg" or fileExtension == ".jpeg" or fileExtension == ".bmp" then
							...
							outputChatBox("#32b3ef[san Pixar]: #ffffffYou started to download the selected image. Please wait the result!", 255, 255, 255, true)
						else
							outputChatBox("#e56b6b[san Pixar]: #ffffffThe selected path extension isn't valid!", 255, 255, 255, true)
						end
					else
						outputChatBox("#e56b6b[san Pixar]: #ffffffFirst add the image URL!", 255, 255, 255, true)
					end
				else
					outputChatBox("#e56b6b[san Pixar]: #ffffffPlease wait #e56b6b" .. 3 - math.floor(deltaTime / 1000) .. " second#ffffff to the next download.", 255, 255, 255, true)
				end
			else
				outputChatBox("#e56b6b[san Pixar]: #ffffffOne download is already in progress!", 255, 255, 255, true)
			end
		end
	end
)
...
addEventHandler("onClientReceiveDownloadedImage", getRootElement(),
	function (imagePixels, errno)
		if errno == 0 then
			...
			outputChatBox("#32b3ef[san Pixar]: #ffffffThe selected image successfully saved.", 255, 255, 255, true)
			outputChatBox("#32b3ef[san Pixar]: #ffffffYou can reach the image on the 'Saved graffitis' panel.", 255, 255, 255, true)
		else
			outputChatBox("#e56b6b[san Pixar]: #ffffffThe selected image not available!", 255, 255, 255, true)
		end
	...
	function (fromClient, pixels, data)
		...
		if graffitiFile then
			...
			if fromClient == localPlayer then
				...
				outputChatBox("#b0e56b[san Pixar]: #ffffffThe new graffiti spraying successfully completed.", 255, 255, 255, true)
			end
		end
	end
)
	...
	function (weapon, _, _, hitX, hitY, hitZ)
		...
		if lastDrawedPixels and not startedGraffiti.isActive and not startedGraffiti.actionDisabled and #drawingSteps >= 2 and getTickCount() >= nextSprayingAfterTick then
			if getTickCount() >= tryToSprayGraffitiTick then
				...
				outputChatBox("#32b3ef[san Pixar]: #ffffffYou started to spray a new graffiti!", 255, 255, 255, true)
			end
				...
				if button == "left" and state == "up" then
					if string.find(selected3DButton, "trash|") then
						...
						if serverGraffitis[fileName].isProtected then
							outputChatBox("#e56b6b[san Pixar]: #ffffffYou can't remove this graffiti because is protected.", 255, 255, 255, true)
							return
						end
					...
					elseif string.find(selected3DButton, "clean|") then
						...
						if not getPedOccupiedVehicle(localPlayer) then
							if not isTimer(cleaningGraffitiTimer) then
								if serverGraffitis[fileName].isProtected then
									outputChatBox("#e56b6b[san Pixar]: #ffffffYou can't clean this graffiti because is protected.", 255, 255, 255, true)
									return
								end
								...
								outputChatBox("#e56b6b[san Pixar]: #ffffffYou are already have an active graffiti cleaning process.", 255, 255, 255, true)
							end
						else
							outputChatBox("#e56b6b[san Pixar]: #ffffffYou can't use this feature while sitting in a vehicle.", 255, 255, 255, true)
						end
					end
				end
			end
...
function cleanGraffiti(fileName)
	...
	outputChatBox("#b0e56b[san Pixar]: #ffffffSuccessfully finished the graffiti cleaning.", 255, 255, 255, true)
end
