	...
	function ()
		...
		if Vehicle then
			if getElementData(localPlayer, "gpsDestination") then
				local destination = getElementData(localPlayer, "gpsDestination")
...
function makeRoute(destinationX, destinationY, uTurned)
	...
	local vehiclePosX, vehiclePosY = getElementPosition(Vehicle)
	...
	local Vehicle = localPlayer
	local vehicleOffsetX, vehicleOffsetY = getPositionFromElementOffset(Vehicle, -1, 0, 0)
	if not gpsRoute or not gpsRoute[1] or not gpsRoute[2] then
		return
	end
	...
	if not uTurned then
		--selectedRouteSound = playSound("radar/gps/sounds/" .. carCanGPSVal .. "/uticel.mpeg")
	end
end
addEvent('criarRota', true)
addEventHandler('criarRota', root, makeRoute)
...
function reRoute(checkShape)
	local Vehicle = localPlayer
	if not gpsRoute or not Vehicle then
		return
	end
...
function playNextGPSSound(sounds, count)
	currentGPSSound = playSound("radar/gps/sounds/" .. carCanGPSVal .. "/" .. sounds[count] .. ".mpeg")
...
function tpWay(player)
	if player and tpWayX ~= 0 and tpWayY ~= 0 then
		local node = getVehicleNodeClosestToPoint(tonumber(tpWayX), tonumber(tpWayY))
		setElementPosition(player, node.x, node.y, node.z + 1)
		if isPedInVehicle(player) then
			local vehicle = getPedOccupiedVehicle(player)
			setElementPosition(vehicle, node.x, node.y, node.z + 1)
		end
		tpWayX = 0
		tpWayY = 0
	end
end
addEvent("class:tpWay", true)
addEventHandler("class:tpWay", root, tpWay)
