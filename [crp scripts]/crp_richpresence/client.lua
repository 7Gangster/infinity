...
function setDiscordRichPresence()
    ...
    if (connected) then
        ...
        --etDiscordRichPresenceState(application['state']);
        if getElementData(localPlayer, 'ID') then 
            setDiscordRichPresenceState( (string.gsub(getPlayerName(localPlayer), '_', ' ') or getElementData(localPlayer, 'ID').. ' #'..getElementData(localPlayer, 'ID') and 'Conectando ao servidor.'));
        else
            setDiscordRichPresenceState('Conectando-se');
        end
...
addEventHandler("addPlayerRichPresence", localPlayer,
    function(data)
        ...
        print(getPlayerName(localPlayer)..' carregou rich presence')
    end, false
);
