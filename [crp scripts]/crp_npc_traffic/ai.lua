...
function initPedRouteData(ped)
	...
	addEventHandler("crp_npc:onNPCTaskDone",ped,continuePedRoute)
end
...
function continuePedRoute(task)
	...
	if speed then
		call(crp_npc,"setNPCDriveSpeed",source,speed/180)
...
function addNodeToPedRoute(ped,nodeid,nb)
	...
	if vehicle then
		local off = speed*0.1
		local enddist = lights and call(crp_npc_coldata,"getModelBoundingBox",model,"y2")+5 or off
		if nb then
			call(crp_npc,"addNPCTask",ped,{"driveAroundBend",node_x[nb],node_y[nb],x1,y1,z1,x2,y2,z2,off,enddist})
		else
			call(crp_npc,"addNPCTask",ped,{"driveAlongLine",x1,y1,z1,x2,y2,z2,off,enddist})
		end
	else
		if nb then
			call(crp_npc,"addNPCTask",ped,{"walkAroundBend",node_x[nb],node_y[nb],x1,y1,z1,x2,y2,z2,1,1})
		else
			call(crp_npc,"addNPCTask",ped,{"walkAlongLine",x1,y1,z1,x2,y2,z2,1,1})
		end
	end
	...
	if lights then
		call(crp_npc,"addNPCTask",ped,{"waitForGreenLight",lights})
	end
end
