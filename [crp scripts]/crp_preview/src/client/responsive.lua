...
function setPedAttack(theElement, theTarget, theWeapon, theAmmo, thePedSlot)
    if theElement then
       ...
       local bx, by, bz = getPedBonePosition(theTarget, 7)
       if distance < 5 then
            setPedRotation(theElement, rotation)
            setPedLookAt(theElement, x, y, z)
            setPedAimTarget(theElement, bx, by, bz)
            givePedWeapon(theElement, theWeapon, theAmmo)
