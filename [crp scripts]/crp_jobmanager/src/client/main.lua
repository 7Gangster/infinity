...
function render (  )
    if pagina == 'inicio' then
        ...
        dxDrawText('Bem vindo, '..string.gsub(getPlayerName(localPlayer), '_', ' '), 536, 252, 392, 12, white, 1, font['roboto']['regular'][10])

        dxDrawText(string.format('%02d:%02d', time.hour, time.minute), 318, 334, 1283, 52, white, 1, font['roboto']['bold'][50], 'center', 'top')
        dxDrawText(''..Dia..' '..MesWeek[''..Mes..'']..' '..Ano..'', 318, 390, 1283, 52, white, 1, font['roboto']['medium'][10], 'center', 'top')
        ...
        dxDrawText('Empregos', 860, 764, 75, 18, tocolor('FFFFFF', 100), 1, font['roboto']['medium'][10])
        ...
        dxDrawText('Grupos', 987, 764, 75, 18, tocolor('FFFFFF', 100), 1, font['roboto']['medium'][10])
    ...
    elseif pagina == 'Empregos' then 
        ...
        dxDrawText('Empregos', 622, 336, 247, 79, white, 1, font['roboto']['bold'][16])

        local index = 0 
        for i,v in ipairs(cfg.jobs) do 
            if index >= scroll['jobs'] and index <= #jobsPosition then
                if not v.vpn then 
                    index = index + 1   
                    ...
                    dxDrawText(v.emprego, (x+676-567), (y+13), 247, 79, white, 1, font['roboto']['regular'][12])

                    dxDrawImage((x+670-567), (y+40), 31, 31, 'src/assets/member.png')
                    dxDrawText(v.maxMembers, (x+701-567), (y+452-406), 247, 79, white, 1, font['roboto']['regular'][10])
                else
                    if vpn then 
                        index = index + 1   
                        local x, y = unpack(jobsPosition[index])
    ...
    elseif pagina == 'Grupos' then 
        ...
        if not getElementData(localPlayer, 'JobGroup') then
            dxDrawImage(553, 317, 69, 69, 'src/assets/jobs.png')
            dxDrawText('Grupos', 622, 336, 247, 79, white, 1, font['roboto']['bold'][16])

            dxDrawRoundedRectangle(1077, 331, 145, 43, 5, isCursorOnElement(1077, 331, 145, 43) and tocolor('59884A', 100) or tocolor('59884A', 80))
            dxDrawText('CRIAR GRUPO', 1077, 331, 145, 43, white, 1, font['roboto']['regular'][10], 'center', 'center')
            dxDrawRoundedRectangle(1234, 331, 115, 43, 5, isCursorOnElement(1234, 331, 115, 43) and tocolor('A3693A', 100) or tocolor('A3693A', 80))
            dxDrawText('DEMITIR-SE', 1234, 331, 115, 43, white, 1, font['roboto']['regular'][10], 'center', 'center')

            dxDrawImage(553, 398, 31, 31, 'src/assets/groups.png')
            dxDrawText('Lista de grupos disponiveis', 587, 403, 207, 24, white, 1, font['roboto']['regular'][10])

            local i = 0
            for k,v in ipairs(groups) do 
                if i >= scroll['groups'] and i < #groupsPosition then
                    i = i +1
                    ...
                    dxDrawText(v.name, (x+61), (y+27), 158, 27, white, 1, font['roboto']['bold'][12])
                    dxDrawText(v.job, (x+61), (y+45), 158, 27, tocolor('BEBEBE'), 1, font['roboto']['regular'][10])

                    dxDrawText('QTD', (x+201), (y+28), 24, 14, tocolor('BEBEBE'), 1, font['roboto']['regular'][10])
                    dxDrawImage((x+187), (y+39), 31, 31, 'src/assets/member.png')
                    dxDrawText(v.members, (x+218), (y+45), 21, 21, tocolor('FFFFFF'), 1, font['roboto']['regular'][10])

                    dxDrawText('MAX', (x+253), (y+28), 24, 14, tocolor('BEBEBE'), 1, font['roboto']['regular'][10])
                    dxDrawImage((x+250), (y+39), 31, 31, 'src/assets/groups.png')
                    dxDrawText(v.maxMembers, (x+281), (y+45), 21, 21, tocolor('FFFFFF'), 1, font['roboto']['regular'][10])
            ...
            dxDrawImage(553, 317, 69, 69, 'src/assets/jobs.png')
            dxDrawText('Seu grupo', 622, 336, 247, 79, white, 1, font['roboto']['bold'][16])
            ...
            if getElementData(localPlayer, 'JobGroup') == getElementData(localPlayer, 'ID') then
                if not getElementData(localPlayer, 'InService') then
                    dxDrawRoundedRectangle(1077, 331, 145, 43, 5, isCursorOnElement(1077, 331, 145, 43) and tocolor('59884A', 100) or tocolor('59884A', 80))
                    dxDrawText('INICIAR SERVIÇO', 1077, 331, 145, 43, white, 1, font['roboto']['regular'][10], 'center', 'center')
                else 
                    if getElementData(localPlayer, 'InService') then
                        ...
                        dxDrawText('CANCELAR SERVIÇO', 1077, 331, 145, 43, white, 1, font['roboto']['regular'][10], 'center', 'center')
                    end
                end
                dxDrawRoundedRectangle(1234, 331, 115, 43, 5, isCursorOnElement(1234, 331, 115, 43) and tocolor('942222', 100) or tocolor('942222', 80))
                dxDrawText('DESFAZER', 1234, 331, 115, 43, white, 1, font['roboto']['regular'][10], 'center', 'center')
            else
                dxDrawRoundedRectangle(1234, 331, 115, 43, 5, isCursorOnElement(1234, 331, 115, 43) and tocolor('942222', 100) or tocolor('942222', 80))
                dxDrawText('SAIR', 1234, 331, 115, 43, white, 1, font['roboto']['regular'][10], 'center', 'center')
            end

            dxDrawImage(553, 398, 31, 31, 'src/assets/groups.png')
            dxDrawText('Lista de membros ('..members..'/'..getJobAssets(getElementData(localPlayer, 'Job')).maxMembers..')', 587, 403, 207, 24, white, 1, font['roboto']['regular'][10])
            ...
            for k,v in ipairs(list) do 
                if i < 4 then
                    ...
                    dxDrawRectangle(x, (y+88), 392, 6, tocolor('D9D9D9'))
                    dxDrawRoundedRectangle(x, y, 392, 92, 5, tocolor('25262B'))
                    dxDrawText(string.gsub(getPlayerName(v), '_', ' '), (x+66), (y+27), 158, 27, white, 1, font['roboto']['bold'][12])
                    if getElementData(v, 'ID') == getElementData(localPlayer, 'JobGroup') then
                        dxDrawImage(x, (y+13), 59, 59, 'src/assets/leader.png')
                        dxDrawText('Lider', (x+66), (y+45), 158, 27, tocolor('BEBEBE'), 1, font['roboto']['regular'][10])
                    else
                        dxDrawImage(x, (y+13), 59, 59, 'src/assets/member.png')
                        dxDrawText('Membro', (x+66), (y+45), 158, 27, tocolor('BEBEBE'), 1, font['roboto']['regular'][10])
                    end
        ...
        if b == 'left' and s == 'down' then 
            if pagina == 'inicio' then 
                ...
                elseif isCursorOnElement(982, 707, 57, 52) then
                    if getElementData(localPlayer, 'Job') then 
                        ...
                        triggerEvent('addBox', localPlayer, 'Você não está empregado. Vá até um emprego e solicite um serviço.', 'error')
                    end
                end
            elseif pagina == 'Empregos' then 
                local index2 = 0 
                for k,v in ipairs(cfg.jobs) do 
                    if index2 >= scroll['jobs'] and index2 < #jobsPosition then
                        if not v.vpn then 
                            index2 = index2 + 1   
            ...
            elseif pagina == 'Grupos' then 
                if not getElementData(localPlayer, 'JobGroup') then
                    local index = 0
                    for k,v in ipairs(groups) do 
                        if index >= scroll['groups'] and index < #groupsPosition then
                            index = index +1
...
addEventHandler('updateVPN', root, function(list)
    vpn = list
end)
...
addEventHandler('onClientRender', root, function ( )
    if #notify > 0 then 
        for i,v in ipairs(notify) do
            ...
            dxDrawText(v.nome, (x+66), (y+17), 228, 21, tocolor(v.color, alpha), 1, font['roboto']['bold'][12], 'left', 'top')
            dxDrawText(v.msg, (x+19), (y+47), 416, 21, tocolor('FFFFFF', alpha), 1, font['roboto']['regular'][10], 'left', 'top', false, true)
            dxDrawText('Agora', (x+371), (y+17), 56, 21, tocolor('FFFFFF', alpha), 1, font['roboto']['regular'][10])
