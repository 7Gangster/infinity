function makeNPCWalkToPos(npc,x,y,z,maxtime)
	...
	local move = true
	if check_cols then
		local box = call(crp_npc_coldata,"createModelIntersectionBox",model,x,y,z,rot)
		local boxprev = call(crp_npc_coldata,"getElementIntersectionBox",npc)
		move = not call(crp_npc_coldata"doesModelBoxIntersect",box,getElementDimension(npc),boxprev)
	end
	if move then
		setElementPosition(npc,x,y,z,false)
		setPedRotation(npc,rot)
		if check_cols then call(crp_npc_coldata,"updateElementColData",npc) end
		return maxtime
	else
...
function makeNPCWalkAlongLine(npc,x1,y1,z1,x2,y2,z2,off,maxtime)
	...
	local move = true
	if check_cols then
		local box = call(crp_npc_coldata,"createModelIntersectionBox",model,x_next,y_next,z_next,rot)
		local boxprev = call(crp_npc_coldata,"getElementIntersectionBox",npc)
		move = not call(crp_npc_coldata,"doesModelBoxIntersect",box,getElementDimension(npc),boxprev)
	end
	if move then
...
function makeNPCWalkAroundBend(npc,x0,y0,x1,y1,z1,x2,y2,z2,off,maxtime)
	...
	local move = true
	if check_cols then
		local box = call(crp_npc_coldata,"createModelIntersectionBox",model,x_next,y_next,z_next,a_next)
		local boxprev = call(crp_npc_coldata,"getElementIntersectionBox",npc)
		move = not call(crp_npc_coldata,"doesModelBoxIntersect",box,getElementDimension(npc),boxprev)
	end
	if move then
...
function makeNPCDriveToPos(npc,x,y,z,maxtime)
	...
	local move = true
	if check_cols then
		local box = call(crp_npc_coldata,"createModelIntersectionBox",model,x,y,z,rz)
		local boxprev = call(crp_npc_coldata,"getElementIntersectionBox",car)
		move = not call(crp_npc_coldata,"doesModelBoxIntersect",box,getElementDimension(car),boxprev)
	end
	if move then
		...
		if check_cols then call(crp_npc_coldata,"updateElementColData",car) end
		return maxtime
	else
...
function makeNPCDriveAlongLine(npc,x1,y1,z1,x2,y2,z2,off,maxtime)
	...
	local move = true
	if check_cols then
		local box = call(crp_npc_coldata,"createModelIntersectionBox",model,x_next,y_next,z_next,rz)
		local boxprev = call(crp_npc_coldata,"getElementIntersectionBox",car)
		move = not call(crp_npc_coldata,"doesModelBoxIntersect",box,getElementDimension(car),boxprev)
	end
	if move then
